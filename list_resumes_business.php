    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="active treeview">
          <a href="list_resumes_business.php">
...
$comps = 0;
$handle = fopen("companies_business.csv", "r");
$companies=array();
if ($handle) {
    while (($line = fgets($handle)) !== false) {
        // process the line read.
...
for($j = 0; $j < sizeof($splitted); $j++){
	$pp = strpos($resume_array[$a], 'cfo');
	$ppp = strpos($resume_array[$a], 'chief financial officer');
	...
	$p = strpos($resume_array[$a], 'vice president');
	if($p === false){
					}
	else{
	...
	$p1 = strpos($resume_array[$a], 'athlete');
	$p2 = strpos($resume_array[$a], 'ncaa');
	$p3 = strpos($resume_array[$a], 'varsity');
